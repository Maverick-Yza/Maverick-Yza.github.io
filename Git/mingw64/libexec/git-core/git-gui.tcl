...
if {[is_Windows]} {
	...
	if {![info exists env(DISPLAY)]} {
		...
		images create photo gitlogo -width 16 -height 16
		...
		images create photo gitlogo32 -width 32 -height 32
...
proc display_file_helper {w path icon_name old_m new_m} {
	...
	if {$new_m eq {_}} {
		...
		incr lno
		$w conf -state normal
		$w images create $lno.0 \
			-align center -padx 5 -pady 1 \
			-name $icon_name \
			-images [mapicon $w $new_m $path]
		$w insert $lno.1 "[escape_path $path]\n"
		$w conf -state disabled
	} elseif {$old_m ne $new_m} {
		$w conf -state normal
		$w images conf $icon_name -images [mapicon $w $new_m $path]
		$w conf -state disabled
	}
}
...
images create bitmap file_plain -background white -foreground black -data {
...
images create bitmap file_mod -background white -foreground blue -data {
#define mod_width 14
...
images create bitmap file_fulltick -background white -foreground "#007000" -data {
...
images create bitmap file_question -background white -foreground black -data {
...
images create bitmap file_removed -background white -foreground red -data {
...
images create bitmap file_merge -background white -foreground blue -data {
...
images create bitmap file_statechange -background white -foreground green -data {
